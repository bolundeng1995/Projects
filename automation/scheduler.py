...
class ScriptManager:
    ...
    def update_script_time(self, file_name, time):
...
class SchedulerCore:
    ...
    def add_job_with_frequency(self, time, job_func, file_name, frequency, weekday=None):
        """Schedule a job with a specified frequency and weekday."""
        if frequency == "daily":
...
class SchedulerApp:
    ...
    def build_gui(self):
        ...
        # Script List
        self.script_list_tree = ttk.Treeview(self.root, columns=("File Name", "Time", "File Location"), show="headings", height=10)
        ...
        self.script_list_tree.column("File Name", width=200)
        self.script_list_tree.column("Time", width=100)
        self.script_list_tree.column("File Location", width=600)
        ...
        self.pause_button = tk.Button(buttons_frame, text="Pause Scheduler", command=self.pause_scheduler, state=tk.DISABLED)
        self.pause_button.grid(row=1, column=1, padx=5)

        self.resume_button = tk.Button(buttons_frame, text="Resume Scheduler", command=self.resume_scheduler, state=tk.DISABLED)
        self.resume_button.grid(row=1, column=2, padx=5)

        self.clear_jobs_button = tk.Button(buttons_frame, text="Clear Scheduled Jobs", command=self.clear_scheduled_jobs)
        self.clear_jobs_button.grid(row=1, column=3, padx=5)

        # Scheduler Status
        self.scheduler_status = tk.Label(self.root, text="Scheduler Status: Not Running", fg="red", font=("Arial", 12, "bold"))
        ...
        # Logs and Output
        tk.Label(self.root, text="Logs and Output:").pack(pady=5)
    ...
    def update_script_tree(self):
        ...
        sorted_scripts = sorted(
            ...
            key=lambda s: s["time"] if s["time"] else "99:99"  # Sort None times to the end
        )
        for script in sorted_scripts:
            file_name = os.path.basename(script["file_path"])
            time_display = script["time"] if script["time"] else "Not Scheduled"
            row_id = self.script_list_tree.insert("", tk.END, values=(file_name, time_display, script["file_path"]))
            if script["time"]:  # Highlight scheduled scripts
    ...
    def update_scheduled_jobs_tree(self):
        ...
        sorted_jobs = sorted(self.scheduler.jobs, key=lambda job: job["time"])
    ...
    def start_scheduler(self):
        ...
        self.update_scheduled_jobs_tree()
        messagebox.showinfo("Scheduler", "Scheduler started and running in the background.")
            ...
            for item in selected:
                ...
                if time:
                    try:
                        ...
                        if self.script_manager.update_script_time(file_name, time):
